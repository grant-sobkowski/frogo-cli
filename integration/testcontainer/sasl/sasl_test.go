...
func TestContainers_SASLTopicLifecycle(t *testing.T) {
	topic := "tc-sasl-topic"

	t.Run("ConfigureProfile", func(t *testing.T) {
		runCmd(t, "config", "set", "brokers", brokerAddr, "--profile", "test")
		runCmd(t, "config", "set", "security-protocol", "sasl_plaintext", "--profile", "test")
