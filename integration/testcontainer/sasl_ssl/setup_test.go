...
func TestMain(m *testing.M) {
	...
	container, err = redpanda.Run(ctx, redpandaImage,
		...
		redpanda.WithNewServiceAccount("test-user", "test-user-password"),
		redpanda.WithSuperusers("test-user"),
	)
	if err != nil {
	...
	tmpDir, err := os.MkdirTemp("", "frogo-testcontainer-sasl-ssl-*")
	if err != nil {
		container.Terminate(ctx)
	...
	// Write the self-signed cert as the CA cert for frogo to trust the broker.
	caCertPath = filepath.Join(tmpDir, "ca.pem")
	if err := os.WriteFile(caCertPath, cert.Bytes, 0644); err != nil {
		container.Terminate(ctx)
		os.RemoveAll(tmpDir)
		fmt.Fprintf(os.Stderr, "failed to write CA cert: %v\n", err)
		os.Exit(1)
	}
